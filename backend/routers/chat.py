...
Executes on FastAPI worker thread pool to ensure non-blocking high-throughput concurrency.
"""
...
def ask_question(request: ChatRequest):
    ...
    Synchronous definition offloads execution to FastAPI thread pool to prevent event loop blocking.
    """
    ...
    logger.info(f"{'='*60}")
    logger.info(f"💬 [{mode}] Question from {request.tenant_id} (model: {request.model}): {request.question}")
...
def get_history(tenant_id: str, limit: int = 20, source_file: Optional[str] = None):
    """
...
def clear_history(tenant_id: str, source_file: Optional[str] = None):
    """
